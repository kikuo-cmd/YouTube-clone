...
async function loadVideos(searchQuery = '') {
    ...
    if (error || !videos || videos.length === 0) {
        videoGrid.innerHTML = `<div class="empty-message">該当する動画がありません。</div>`;
        shortsGrid.innerHTML = `<div class="empty-message">該当する動画がありません。</div>`;
        return;
    }
    ...
    if (!hasNormalVideo) videoGrid.innerHTML = `<div class="empty-message">該当する動画がありません。</div>`;
